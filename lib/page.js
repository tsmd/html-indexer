...
module.exports = class PageIndexer extends Indexer {
  constructor (settings) {
    super(settings)
    this.exports = ['page', 'page_0', 'page_url', 'page_url_ext']
  }
  ...
  classify (context, next) {
    ...
    for (let i = 0; i < pageMax; i += 1) {
      ...
      context.page_url_ext = i === 0 ? '' : `${context.page_url}.html`
      ...
      if (!isFirst) {
        ...
        context.prev_url_ext = i - 1 === 0 ? '' : `${context.prev_url}.html`
      }
      if (!isLast) {
        context.next = i + 2
        context.next_0 = i + 1
        context.next_url = i + 2
        context.next_url_ext = `${context.next_url}.html`
      }
